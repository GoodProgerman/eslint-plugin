  ...
  create(context) {
   ...
   return {
      ImportDeclaration(node) {
			...
			if(importIsNotFromPublicApi && !isTestFile){
				context.report({
					node: node, 
			...
			if(!isTestingPublicApi && isTestFile) {
				context.report({node, messageId: 'necessaryImportFromTestingPublicApi'})
			}

			if(isTestingPublicApi && !isTestFile) {
				context.report({node, messageId: 'forbiddenToImportFromTestingPublicApi'})
			}
		}
   };
  },
